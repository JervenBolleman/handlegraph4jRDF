...
public class FALDO {
    ...
    /**
     * Used to describe a location that consists of a number of Regions but
     * where the order is not known. e.g. the oddly named order() keyword in a
     * INSDC file. *
     */
     ...
     * being on 'both' strands of a double-stranded sequence, rather than one or
     * the other. *
     */
    public static final IRI BothStrandsPosition = VF.createIRI(NAMESPACE, "BothStrandsPosition");

    /**
     * Sometimes a location of a feature is defined by a collection of regions
     * e.g. join() and order() in INSDC records. One should always try to model
     * the semantics more accurately than this, these are fallback options to
     * encode legacy data. *
     */
    ...
    /**
     * The position is on the forward (positive, 5' to 3') strand. Shown as a
     * '+' in GFF3 and GTF. *
     */
    ...
    /**
     * This indicates that a feature is between two other positions that are
     * both known exactly and that are next to each other. An example is a
     * restriction enzyme cutting site. The cut is after one position and before
     * the other position (hence, in between). *
     */
    public static final IRI InBetweenPosition = VF.createIRI(NAMESPACE, "InBetweenPosition");

    /**
     * Use when you have an idea of the range in which you can find the
     * position, but you cannot be sure about the exact position. *
     */
     ...
     * complete)."^^xsd:string , "Should be used when the location of a region
     * is defined by an ordered list of Regions. However, try to avoid using
     * these types in favor of using more explicit semantics about why the order
     * is important. *
     */
    public static final IRI ListOfRegions = VF.createIRI(NAMESPACE, "ListOfRegions");

    /**
     * The position is known to be one of the more detailed positions listed by
     * the location predicate. *
     */
    public static final IRI OneOfPosition = VF.createIRI(NAMESPACE, "OneOfPosition");

    /**
     * Superclass for the general concept of a position on a sequence. The
     * sequence is designated with the reference predicate. *
     */
    ...
    /**
     * The position is on the reverse (complement, 3' to 5') strand of the
     * sequence. Shown as '-' in GTF and GFF3. *
     */
     ...
     * found. If you do not yet know which stand the feature is on, you should
     * tag the position with just this class. If you know more you should use
     * one of the subclasses. This means a region described with a '.' in GFF3.
     * A GFF3 unstranded position does not have this type in FALDO -- those are
     * just a 'position'. *
     */
     ...
     * Only used in the InBetweenPosition to say it is after a nucleotide, but
     * before the next one. *
     */
    public static final IRI after = VF.createIRI(NAMESPACE, "after");

    /**
     * This predicate is used to indicate that the feature is found before the
     * exact position. Use to indicate, for example, a cleavage site. The
     * cleavage happens between two amino acids before one and after the other.
     * *
     */
    ...
    /**
     * This is the inverse of the begin:property. It is included to make it
     * easier to write a number of OWL axioms. You should rarely use this in
     * your raw data. *
     */
     ...
     * write a number of OWL axioms. You should rarely use this in your raw
     * data. *
     */
     ...
     * and its range or position. For example, when annotating the region that
     * describes an exon, the exon would be the subject and the region would be
     * the object of the triple or: 'active site' 'location' [is] 'position 3'.
     * *
     */
    public static final IRI location = VF.createIRI(NAMESPACE, "location");

    /**
     * Denoted in 1-based closed coordinates, i.e. the position on the first
     * amino acid or nucleotide of a sequence has the value 1. For nucleotide
     * sequences we count from the 5'end of the sequence, while for Aminoacid
     * sequences we start counting from the N-Terminus."^^xsd:string , "The
     * position value is the offset along the reference where this position is
     * found. Thus the only the position value in combination with the reference
     * determines where a position is. *
     */
    public static final IRI position = VF.createIRI(NAMESPACE, "position");
